...
# 2026 Verified Dataset IDs - L3S is usually the most stable for history
DATASETS = [
...
def get_variable_name(ds_id):
    ...
    try:
        ...
        pass
    return "sea_surface_temperature" # Default fallback
    ...
    for ds in DATASETS:
        ...
        try:
            ...
            if resp.status_code != 200:
                print(f"  Timeline fetch failed ({resp.status_code}). Trying next dataset.")
                continue
            
            all_timestamps = [row[0] for row in resp.json()['table']['rows']]
            target_timestamps = all_timestamps[-MAX_DAYS:]
            
            # Step 2: Identify the correct variable name
            ...
            for ts in target_timestamps:
                ...
                if download_data(ds_id, ts, var_name, filepath):
                    ...
                    print(f"  Failed to download {clean_date}")
